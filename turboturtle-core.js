/* turboturtle-core.js
   ...
   - Parallax tweens
   - Jetplane arc
   - Galaxy: fixed layer + clip-path synced to .about_underwater (NO SVG mask)
...
(function (root) {
  ...
  function startCore() {
    ...
    // ─────────────────────────────────────
    // Parallax & tweens (your original ones)
    // ─────────────────────────────────────
    ...
    gsap.to(".about_rocket", { x: 130 * vw, y: -20 * vw, ease:"none",
      scrollTrigger:{ trigger: ".parallax-wrapper", start: function(){ return innerHeight*0.4+"px top"; },
                      end: function(){ return innerHeight*0.7+"px top"; }, scrub: true, invalidateOnRefresh: true }});
    ...
    // ─────────────────────────────────────
    // GALAXY: fixed layer + clip-path that follows .about_underwater
    // (replaces previous pin/mask approach)
    // ─────────────────────────────────────
    (function initGalaxyClip() {
      var el = document.querySelector(".about_galaxy");
      var underwater = document.querySelector(".about_underwater");
      if (!el || !underwater) return;

      // Parallax depth (smaller = slower = deeper)
      var ratio = isMobile ? 0.003 : 0.006;

      // Avoid conflicting transforms by always setting via gsap.set
      gsap.set(el, { y: 0, force3D: true });

      // Compute clip-path inset() from underwater's live viewport rect
      function updateClip() {
        ...
        if (x2 <= x1 || y2 <= y1) {
          // No intersection -> hide galaxy completely
          el.style.clipPath = "inset(100% 0 0 100%)";
        } else {
          var top    = y1;
          var left   = x1;
          var right  = innerWidth  - x2;
          var bottom = innerHeight - y2;
          el.style.clipPath = "inset(" + top + "px " + right + "px " + bottom + "px " + left + "px)";
        }
      }

      // Update parallax & clip on the same frame
      var ticking = false;
      function requestUpdate() {
        ...
        requestAnimationFrame(function () {
          ticking = false;
          // Slow vertical drift
          var s = (typeof lenis?.scroll === "number") ? lenis.scroll : (root.pageYOffset || 0);
          var y = -s * ratio; // negative to drift upward as you scroll down
          gsap.set(el, { y: y });
      ...
      // Hooks
      if (lenis && lenis.on) lenis.on("scroll", requestUpdate);
      root.addEventListener("resize", requestUpdate);
      ScrollTrigger && ScrollTrigger.addEventListener("refresh", requestUpdate);
